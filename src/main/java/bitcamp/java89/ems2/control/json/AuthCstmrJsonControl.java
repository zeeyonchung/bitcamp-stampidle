...
@RestController
public class AuthCstmrJsonControl {
  ...
  public AjaxResult login(String email, String password, String userType,
    ...
    if (member == null) {
      return new AjaxResult(AjaxResult.FAIL, "이메일 또는 암호가 틀리거나, 가입된 회원이 아닙니다.cstmr");
    }
    
    session.setAttribute("member", member); // HttpSession에 저장한다.
    return new AjaxResult(AjaxResult.SUCCESS, "로그인 성공!cstmr");
  }
  ...
  public AjaxResult logout(HttpSession session) throws Exception {
    ...
    return new AjaxResult(AjaxResult.SUCCESS, "로그아웃 성공입니다.cstmr");
  }
  ...
  public AjaxResult loginUser(HttpSession session) throws Exception {
    ...
    if (member == null) { // 로그인이 되지 않은 상태
      return new AjaxResult(AjaxResult.FAIL, "로그인을 하지 않았습니다.cstmr");
    }
